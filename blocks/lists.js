...
Blockly.Blocks['lists_positional1'] = {
  /**
...
//    this.setNextStatement(true, null);
  ...
  compose: function(containerBlock) {
    ...
    var i=0;
    while (itemBlock) {
//      alert("i= " + i + " blockid = " + itemBlock.id + " "  + this.detection);
        if (i!=0 && itemBlock.id!=this.mem[i] && this.detection==0) {
                                                this.detection=i;
        }
//   alert(this.mem[i]);
//   itemBlock.id=i;
        this.mem[i]=itemBlock.id;
//      alert("detection = " + this.detection + " mem = " + this.mem[i]);
        connections.push(itemBlock.valueConnection_);
        itemBlock = itemBlock.nextConnection &&
        itemBlock.nextConnection.targetBlock();
        i++;
    }
//  alert("detection = " + this.detection);
    this.itemDelete=false;
    this.itemAdd=false;
    if(i<this.itemCount_){ this.itemDelete=true }
    if(i>this.itemCount_){ this.itemAdd=true }    
  ...
  updateShape_: function() {
    ...
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput('ADD' + i)) {
	  	if (i==0){
			var input = this.appendValueInput('ADD' + i)
//		    .appendField(new Blockly.FieldTextInput("default"), "a")
			.setAlign(Blockly.ALIGN_RIGHT)
            .appendField(new Blockly.FieldTextInput("default"), "b" + i)
			.appendField("=>", "c" + i);
			} else {
			 var input = this.appendValueInput('ADD' + i)
		    .setAlign(Blockly.ALIGN_RIGHT)
